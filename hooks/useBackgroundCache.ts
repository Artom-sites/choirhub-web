...
import { useRepertoire, runIdle } from '@/contexts/RepertoireContext';
...
export function useBackgroundCache() {
    ...
    useEffect(() => {
        ...
        if (repertoireSongs.length > 0) {
            // A bit of delay to let the UI finish rendering Repertoire UI first
            setTimeout(() => {
                cacheUpcomingServices();
            }, 500);
        }
    }, [userData?.choirId, repertoireSongs.length, cacheServiceSongs, progress.isRunning, router]);
}
